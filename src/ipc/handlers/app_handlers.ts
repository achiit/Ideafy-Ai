...
export async function createApp(
  ...
  logger.info(`[createApp] Starting with name: ${params.name}`);
  ...
  // Create a new app
  logger.info(`[createApp] Inserting app into database...`);
  const [app] = await db
    .insert(apps)
    .values({
      name: params.name,
    ...
    .returning();
  logger.info(`[createApp] App inserted.`);

  // Create an initial chat for this app
  logger.info(`[createApp] Inserting chat into database...`);
  const [chat] = await db
    .insert(chats)
    .values({
      appId: app.id,
    ...
    .returning();
  logger.info(`[createApp] Chat inserted.`);

  logger.info(`[createApp] Creating from template...`);
  await createFromTemplate({
    fullAppPath,
  });
  logger.info(`[createApp] Created from template.`);

  // Initialize git repo and create first commit
  logger.info(`[createApp] Initializing git repo...`);
  await git.init({
    fs: fs,
    dir: fullAppPath,
    defaultBranch: "main",
  });
  logger.info(`[createApp] Git repo initialized.`);

  // Stage all files
  logger.info(`[createApp] Staging files...`);
  await git.add({
    fs: fs,
    dir: fullAppPath,
    filepath: ".",
  });
  logger.info(`[createApp] Files staged.`);

  // Create initial commit
  logger.info(`[createApp] Creating initial commit...`);
  const commitHash = await gitCommit({
    path: fullAppPath,
    message: "Init Dyad app",
  });
  logger.info(`[createApp] Commit created.`);

  // Update chat with initial commit hash
  logger.info(`[createApp] Updating chat with commit hash...`);
  await db
    .update(chats)
    .set({
  ...
  logger.info(`[createApp] Chat updated.`);

  logger.info(`[createApp] Finished for: ${params.name}`);
